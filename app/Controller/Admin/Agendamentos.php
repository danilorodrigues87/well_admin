...
class Agendamentos extends Page
{
    ...
    public static function list($request): string
    {
        ...
        while ($row = $stmt->fetch(\PDO::FETCH_ASSOC)) {
            ...
            $saldoPlano = $planoId > 0 ? PlanoService::totalSaldoIncluso($planoId) : 0.0;
            $itens .= '<tr>
                <td>'.CrudHelper::e($c->nome_fantasia).$urgente.'</td>
                <td class="small">'.CrudHelper::e($c->plano_nome ?: '—').'</td>
                <td>'.CrudHelper::e($c->cidade).'</td>
                <td>'.$data.'</td>
                <td>'.($planoId > 0 ? number_format($saldoPlano, 2, ',', '.').' kg' : '—').'</td>
                <td>
